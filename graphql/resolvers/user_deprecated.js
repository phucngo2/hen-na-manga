...
const { deleteAvatar, writeAvatar } = require("../../utils/filesHandler");
    ...
    Mutation: {
        updateAvatar: async (parent, { userId, avatar }, context) => {
            ...
            if (existUser.avatar) {
                ...
                fileName = avatarArr[avatarArr.length - 1];
                ...
                while (true) {
                    ...
                    const existNameCount = await User.countDocuments({
                        avatar: `${SERVER_URL}/avatar/${fileName}.jpg`,
                    });

                    if (existNameCount === 0) break;
                }
            }

            const newAvatarUrl = await writeAvatar(avatarFile, fileName);
